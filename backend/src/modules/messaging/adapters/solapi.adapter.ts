...
export class SolapiAdapter {
  ...
  private async initializeSolapi() {
    try {
      // 데이터베이스에서 API 키 가져오기
      this.apiKey = await this.settingsService.getActualApiKey('SOLAPI_API_KEY');
      this.apiSecret = await this.settingsService.getActualApiKey('SOLAPI_API_SECRET');

      if (!this.apiKey || !this.apiSecret) {
        this.logger.warn('솔라피 API 키가 설정되지 않았습니다. 테스트 모드로 실행됩니다.');
        return;
      }
  ...
  async sendSMS(payload: MessagePayload): Promise<SendResult> {
    try {
      ...
      const sender = await this.settingsService.getActualApiKey('SOLAPI_SENDER');
    ...
    try {
      ...
      if (payload.imageUrl) {
        ...
        if (!fs.existsSync(imagePath)) {
          this.logger.warn(`Image file not found: ${imagePath}, sending as LMS`);
        ...
        const imageId = await this.uploadImageToSolapi(imageBuffer, path.basename(imagePath));
   ...
   * 이미지를 솔라피 스토리지에 업로드
   */
  private async uploadImageToSolapi(imageBuffer: Buffer, filename: string): Promise<string | null> {
    try {
  ...
  async sendKakaoAlimtalk(payload: MessagePayload): Promise<SendResult> {
    try {
      ...
      const pfId = await this.settingsService.getActualApiKey('SOLAPI_KAKAO_PFID');
      ...
      this.logger.error(`Kakao Alimtalk send failed: ${error.message}`, error.stack);
  ...
  async sendKakaoFriendtalk(payload: MessagePayload): Promise<SendResult> {
    try {
      ...
      this.logger.error(`Kakao Friendtalk send failed: ${error.message}`, error.stack);
