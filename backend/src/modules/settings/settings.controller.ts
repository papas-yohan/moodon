...
import { SettingsService, ApiKeySettings, NotificationSettings, MessageTemplate } from './settings.service';
...
export class SettingsController {
  ...
  @ApiParam({ name: 'type', description: 'API 키 타입 (SOLAPI_API_KEY, SOLAPI_API_SECRET, SOLAPI_SENDER, SOLAPI_KAKAO_PFID)' })
  ...
  async updateApiKey(
    ...
    return { 
      success: true,
      message: 'API 키가 성공적으로 업데이트되었습니다. 변경사항을 적용하려면 서버를 재시작하거나 /api/v1/settings/reload-api-keys를 호출하세요.'
    };
  }
  ...
  async reloadApiKeys(): Promise<{ success: boolean; message: string }> {
    ...
    return {
      success: true,
      message: 'API 키 재로드가 요청되었습니다. 다음 발송부터 새로운 설정이 적용됩니다.'
    };
  }
  ...
  @ApiParam({ name: 'id', description: '템플릿 ID' })
  async deleteMessageTemplate(@Param('id') id: string): Promise<{ success: boolean }> {
