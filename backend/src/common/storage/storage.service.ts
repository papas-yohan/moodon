...
export class StorageService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.logger.log(`Storage configuration: USE_S3 env=${useS3Env}, useS3=${this.useS3}, uploadDir=${this.uploadDir}`);
  ...
  private async initializeLocalStorage() {
    try {
      ...
      await fs.mkdir(path.join(this.uploadDir, 'composed'), { recursive: true });
    ...
    try {
      ...
      if (options?.resize || options?.quality) {
        try {
          ...
          this.logger.error('Sharp processing failed, using original buffer', sharpError);
